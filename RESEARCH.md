...
## 23. First user-testing feedback and revisions

### Feedback received

Early beta feedback identified two actionable usability needs:

1. The purpose and importance of the recovery code were not explained early enough in the contribution journey.
2. A dark viewing mode was requested for more comfortable reading.

### Evaluation and decisions

| Finding | Priority | Decision |
|---|---:|---|
| Recovery-code purpose is unclear | High | Revise immediately because it affects contributor control and the ability to remove an anonymous post. |
| Dark mode is unavailable | Medium | Add during this iteration because it is a contained interface change that supports reading comfort without altering the core experience. |

### Revisions implemented

- Added recovery-code guidance beside the contribution steps so contributors learn about it before writing.
- Added a prominent explanation on the final privacy-check screen stating when the code appears, where it is used, and why Unsaid cannot retrieve a lost code.
- Revised the confirmation screen to tell contributors to save the code before leaving and linked directly to the **Remove a contribution** page.
- Preserved the existing one-click copy control and clarified that the code is the only key to a post because no account is created.
- Added a light/dark mode control to the global header so it is available on every page.
- Used the visitor's saved choice on later visits; when no choice exists, the site initially follows the device colour preference.
- Applied the dark palette through the existing design tokens so pages, fields, borders, buttons, and story content remain visually consistent.

### Questions for the next testing round

- Without help, can a contributor explain what the recovery code does?
- Can they identify where to use the code if they want a post removed?
- Do they understand that a lost recovery code cannot be retrieved?
- Can readers find and operate the theme control?
- Does the selected theme remain after refreshing and moving between pages?
- Is every page readable and comfortable in both themes?

These changes should be evaluated with the remaining testers before treating either issue as resolved.
